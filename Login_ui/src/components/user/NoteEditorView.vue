...
const handleAction = async (action, noteId) => {
  ...
  try {
    if (action === '重命名') {
      ...
      // **调用自定义下载弹窗**
      const isConfirmed = await showDownloadDialog(noteId, note.title);
      ...
      if (isConfirmed) {
        ...
        const downloadUrl = await getFileUrl(fileName); // 🚀 调用 getFileUrl
        if (!downloadUrl) {
        ...
        // 使用笔记的 title 作为下载的文件名
        const fileExtension = note.fileType ? `.${note.fileType.toLowerCase()}` : '.md';
        ...
        console.log(`Note ${noteId} download triggered from URL: ${downloadUrl}`);
        alert(`笔记 "${note.title}" 下载已开始。`);
      }
